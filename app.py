...
TREE_DATABASE = {
    ...
    "African Locust Bean (Kalwa)": {"botanical": "Parkia biglobosa", "wd": 0.62, "gf": 0.021, "type": "Indigenous Shade"},
    "Acacia nilotica (Bagaruwa)": {"botanical": "Vachellia nilotica", "wd": 0.80, "gf": 0.019, "type": "Desert Control"},
    ...
    "African Mahogany (Rainforest)": {"botanical": "Khaya ivorensis", "wd": 0.65, "gf": 0.022, "type": "Timber & Canopy"},
    ...
    "Terminalia (Madagascar Almond)": {"botanical": "Terminalia mantaly", "wd": 0.55, "gf": 0.038, "type": "Roadside Shade"}
}
...
def generate_smart_recommendations(df_batches):
    ...
    messages.append(f"🌱 **Top Performer:** *{top_tree}* has the highest survival rate at **{top_percentage:.1f}%**. Consider planting more of this type!")
    ...
    if not failed_trees.empty:
        ...
        messages.append(f"⚠️ **Attention Needed:** Tree types like {problem_list} are showing survival rates below 50%. Check soil quality or watering frequency.")
...
if active_screen == "1. School Form (Record Planting)":
    ...
    with st.expander("📍 Automatic GPS Verification Status", expanded=True):
        ...
        st.caption("🔒 *Security Note: GPS coordinates are automatically locked via your device to prevent location errors.*")
    ...
    if st.session_state.temporary_tree_list:
        ...
        st.dataframe(temp_preview_df.rename(columns={"variety":"Tree Variety", "planted":"Total Planted", "survived":"Total Survived"}), use_container_width=True, hide_index=True)
    ...
    with st.form("final_report_form"):
        ...
        local_lga = col_m1.selectbox("Your Local Government Area (LGA):", ["Katsina", "Batagarawa", "Daura", "Funtua", "Dutsin-Ma"])
        ...
        if send_report_to_database:
            if not st.session_state.temporary_tree_list:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (current_user, local_lga, "Katsina State", str(planting_day), locked_lat, locked_lon, gps_signals, "EcoTrack Web App Engine v2", saved_files_string))
...
# ==============================================================================
    ...
    if all_data.empty:
        ...
        dropdown_options = {
            row["event_id"]: f"Project #{row['event_id']} by {row['account_name']} ({row['lga_name']}) — Age: {row['current_age']}"
            for _, row in grouped_records.iterrows()
        }
        
        target_project_id = st.selectbox("Select a Project Record to Inspect or Review:", list(dropdown_options.keys()), format_func=lambda x: dropdown_options[x])
        ...
        with col_p1:
            ...
            st.write(f"**Geotagged Coordinates:** {project_trees_subset['latitude'].iloc[0]}, {project_trees_subset['longitude'].iloc[0]}")
        
        with col_p2:
            st.markdown("#### 🪵 Current Tree Counts on Record")
            st.dataframe(project_trees_subset[["tree_species", "qty_planted", "qty_survived"]].rename(columns={"tree_species":"Tree Variety", "qty_planted":"Planted", "qty_survived":"Living"}), hide_index=True, use_container_width=True)

        st.markdown("### 📝 Log a New Inspection Progress Check")
        with st.form("new_checkpoint_form"):
            col_c1, col_c2, col_c3 = st.columns(3)
            milestone_step = col_c1.selectbox("Time Horizon Milestone Point:", ["1 Month Check", "3 Months Check", "6 Months Check", "12 Months Check", "24 Months Check", "36 Months Check"])
        ...
        conn = sqlite3.connect(DB_FILE)
        past_checkpoints = pd.read_sql_query("SELECT * FROM checkpoints WHERE event_id=?", conn, params=(target_project_id,))
        conn.close()
        
        if past_checkpoints.empty:
            st.info("No prior progress checks recorded for this project yet.")
        else:
            st.dataframe(past_checkpoints[["interval_months", "qty_alive", "qty_dead", "inspection_date", "inspector", "verification_status"]].rename(columns={"interval_months":"Milestone", "qty_alive":"Alive Count", "qty_dead":"Dead Count", "inspection_date":"Checked On", "inspector":"Verified By", "verification_status":"Status"}), use_container_width=True, hide_index=True)
...
# ==============================================================================
    ...
    if analytics_base.empty:
        ...
        with st.expander("🎛️ Filter Dashboard Content by Region or Tree Variety", expanded=True):
            ...
            selected_states = col_f1.multiselect("Select State Region:", analytics_base["state_name"].unique(), default=analytics_base["state_name"].unique())
            selected_lgas = col_f2.multiselect("Select LGA District:", analytics_base["lga_name"].unique(), default=analytics_base["lga_name"].unique())
            selected_trees = col_f3.multiselect("Select Tree Variety:", analytics_base["tree_species"].unique(), default=analytics_base["tree_species"].unique())
        ...
        if filtered_view.empty:
            ...
            map_plot_data = filtered_view[["latitude", "longitude", "account_name", "qty_survived"]].dropna().rename(columns={"latitude": "lat", "longitude": "lon"})
            st.map(map_plot_data, use_container_width=True)
            
            # Performance and Download Data Tabs Section
            st.markdown("---")
            tab_charts, tab_lga, tab_reports = st.tabs(["🌳 Tree Performance Analysis", "🏆 LGA Performance Area", "💾 Document & Report Downloader"])
            
            with tab_charts:
                col_c1, col_c2 = st.columns(2)
                with col_c1:
                    st.write("##### Total Living Trees by Variety Type")
                    variety_counts = filtered_view.groupby("tree_species")["qty_survived"].sum().sort_values(ascending=False)
                    st.bar_chart(variety_counts, color="#1E5631")
                with col_c2:
                    st.write("##### Total Carbon CO2 Absorption by Variety Type")
                    variety_carbon = filtered_view.groupby("tree_species")["co2_absorbed_kg"].sum().sort_values(ascending=False)
            ...
            with tab_reports:
                ...
                if active_tier in ["Enterprise Tier", "Climate-Smart Academy"]:
                    ...
                    st.warning("🔒 **Subscription Plan Notice:** High-density automated document downloading requires upgrading your account to the Climate-Smart Academy Tier or higher.")
...
# ==============================================================================
    ...
    if ranking_base.empty:
        ...
        def assign_award_status(rank):
            if rank == 1: return "🥇 Champion Team"
            elif rank == 2: return "🥈 Premium Tier"
            elif rank == 3: return "🥉 Merit Tier"
        ...
        st.dataframe(
            friendly_leaderboard[["School Name/Institution", "Trees Planted", "Trees Living", "Survival Rate (%)", "Total Impact Score", "Achievement Award"]],
